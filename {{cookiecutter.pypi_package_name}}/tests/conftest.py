...
load_dotenv()
RUN_TIMEOUT = float(os.getenv("PYTEST_TIMEOUT", "60"))
log_path_env = os.getenv("PYTEST_LOG")
log_path = (
...
def pytest_sessionstart(session: pytest.Session) -> None:
    session.name = "{{ cookiecutter.project_name }} Tests"
...
def log_session_start_and_end(request: pytest.FixtureRequest) -> Generator[None]:
    ...
    env_table.add_row(
        "Plugins",
        " ".join(list(config.invocation_params.plugins))
...
def log_test_start_and_end(request: pytest.FixtureRequest) -> Generator[None]:
    ...
    if hasattr(request.node, "rep_call"):
        outcome: pytest.TestReport = request.node.rep_call
        if outcome.passed:
            status = "[green]✓ PASSED[/]"
        elif outcome.failed:
            status = "[red]✗ FAILED[/]"
